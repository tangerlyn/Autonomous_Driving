...
#include "pid_controller.hpp" // for pid controller
...
PurePursuit::PurePursuit() : Node("pure_pursuit_node") {
    ...
    pid_ = std::make_shared<PIDController>(kp, ki, kd);
    last_time_ = this->now();
    // end pid controller
...
// for pid controller
double PurePursuit::get_curvature() {
    int i = waypoints.velocity_index;
    if (i < 1 || i + 1 >= (int)waypoints.X.size()) return 0.0;

    double x0 = waypoints.X[i - 1], y0 = waypoints.Y[i - 1];
    double x1 = waypoints.X[i],     y1 = waypoints.Y[i];
    double x2 = waypoints.X[i + 1], y2 = waypoints.Y[i + 1];
    ...
    if (len1 < 1e-4) return 0.0;

    double angle = std::atan2(cross, dot);
    return std::abs(angle) / len1;
}

void PurePursuit::adjust_pid_by_curvature(double curvature) {
    if (curvature > 1.0) {
        pid_->set_gains(2.5, 0.1, 0.2);
    } else if (curvature > 0.3) {
        pid_->set_gains(1.5, 0.05, 0.1);
    } else {
        pid_->set_gains(1.0, 0.0, 0.05);
    }
}
// end pid controller
...
void PurePursuit::publish_message(double steering_angle, double speed) {
    ...
    drive_msgObj.drive.speed = speed;
    // end pid controller
...
void PurePursuit::odom_callback(const nav_msgs::msg::Odometry::ConstSharedPtr odom_submsgObj) {
    ...
    // for pid controller
    double current_speed = odom_submsgObj->twist.twist.linear.x;
    ...
    this->adjust_pid_by_curvature(curvature);

    double error = target_speed_ - current_speed;
    double control_speed = pid_->update(error, dt);

    double steering_angle = p_controller();

    publish_message(steering_angle, control_speed);
    // end pid controller
