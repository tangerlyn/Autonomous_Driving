...
// 이 헤더 파일 추가하고 밑에 2가지 부분 추가하면 됩니다.
...
// 웨어 포인트에서의 Yaw(진행 각도)와 Curvature(곡률) 벡터 추가했습니다.

    waypoints.Yaw.resize(num_waypoints);
    waypoints.Curvature.resize(num_waypoints);

    for (int i = 0; i < num_waypoints - 1; ++i) {
        double dx = waypoints.X[i + 1] - waypoints.X[i];
        double dy = waypoints.Y[i + 1] - waypoints.Y[i];
        waypoints.Yaw[i] = std::atan2(dy, dx);
        waypoints.Curvature[i] = 0.0;  
    }
    waypoints.Yaw[num_waypoints - 1] = waypoints.Yaw[num_waypoints - 2];

    // 수치 미분 기반으로 곡률 계산하도록 했습니다.
        for (int i = 1; i < num_waypoints - 1; ++i) {
        double dx1 = waypoints.X[i] - waypoints.X[i - 1];
        double dy1 = waypoints.Y[i] - waypoints.Y[i - 1];
        double dx2 = waypoints.X[i + 1] - waypoints.X[i];
        double dy2 = waypoints.Y[i + 1] - waypoints.Y[i];
        ...
        if (denominator != 0)
            waypoints.Curvature[i] = numerator / denominator;
        else
            waypoints.Curvature[i] = 0.0;
    }

    waypoints.Curvature[0] = waypoints.Curvature[1];
    waypoints.Curvature[num_waypoints - 1] = waypoints.Curvature[num_waypoints - 2];

    // missing velocity → 0.0으로 채웠습니다.
    if (waypoints.V.size() < num_waypoints) {
        waypoints.V.resize(num_waypoints, 0.0);  
    }
...
// 여기 조향각 계산 부분을 lqrSteeringControl()로 계산하도록 바꿔놨습니다.
// lqrSteeringControl()는 lqr_control.cpp 보시면 됩니다.
// 여기서 결과로 나온 delta를 조향각 steering_angle에 할당했습니다.
...
void PurePursuit::odom_callback(const nav_msgs::msg::Odometry::ConstSharedPtr odom_submsgObj) {
    ...
    yaw_car_world = tf2::getYaw(odom_submsgObj->pose.pose.orientation);
    ...
    auto [delta, ind, e, th_e] = lqrSteeringControl(
        waypoints.X,
        waypoints.Y,
        waypoints.Yaw,
        waypoints.Curvature,
        x_car_world, y_car_world, yaw_car_world, curr_velocity,
        prev_error, prev_heading_error
    );
    steering_angle = delta;
    prev_error = e;
    prev_heading_error = th_e;
